...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0) {
			throw new TreeException("can't add line");
		}
		...
		else
		{
			...
			if(parentNode == null) {
				throw new TreeException("can't find parent");
			}
			
		}
		
		// Add child nodes to parentNode.
		for(String currentChild: childrenArray) {
			TreeNode addNode = new TreeNode(currentChild);
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null) {
			throw new TreeException("can't find first name");
		}
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null) {
			throw new TreeException("can't find second name");
		}		
		...
		System.out.println("------------------");
		for(TreeNode ele: ancestorsOf1) {
			System.out.println(ele.name);
		}
		System.out.println("------------------");
		for(TreeNode ele: ancestorsOf2) {
			System.out.println(ele.name);
		}
